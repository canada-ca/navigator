...
Navigator SHALL allow a workspace owner to declare the highest Deliberate Threat Level (Td1–Td7) the organization realistically defends against, and SHALL display that scope throughout the workspace.
...
#### Scenario: Max Td level is shown in workspace context
- **WHEN** a user views any workspace area (threats, assumptions, mitigations, etc.)
- **THEN** Navigator displays the workspace's max Td level if set, so collaborators understand the agreed defensive scope
