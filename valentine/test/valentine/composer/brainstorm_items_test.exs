defmodule Valentine.Composer.BrainstormItemsTest do
  ...
  describe "list_brainstorm_items/2" do
    ...
    test "filters by type" do
      workspace = workspace_fixture()
      {:ok, threat_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "A threat"
      })
      {:ok, _assumption_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :assumption,
    ...
    test "filters by status" do
      workspace = workspace_fixture()
      {:ok, draft_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Draft item"
      })
      {:ok, _clustered_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
  ...
  describe "list_brainstorm_items_by_type/2" do
    test "groups items by type" do
      workspace = workspace_fixture()
      {:ok, threat1} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "First threat"
      })
      {:ok, threat2} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Second threat"
      })
      {:ok, assumption1} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :assumption,
        raw_text: "First assumption"
      })
  ...
  describe "list_cluster_items/2" do
    test "returns items in a specific cluster" do
      workspace = workspace_fixture()
      {:ok, cluster_item1} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Clustered item 1",
        cluster_key: "cluster_123"
      })
      {:ok, cluster_item2} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Clustered item 2",
        cluster_key: "cluster_123"
      })
      {:ok, _other_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
  ...
  describe "list_assembly_candidates/2" do
    test "returns items with clustered or candidate status" do
      workspace = workspace_fixture()
      {:ok, clustered_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Clustered item",
        status: :clustered
      })
      {:ok, candidate_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Candidate item",
        status: :candidate
      })
      {:ok, _draft_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Draft item",
        status: :draft
    ...
    test "filters by cluster_key when provided" do
      workspace = workspace_fixture()
      {:ok, target_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
  ...
  describe "list_backlog_items/2" do
    test "returns items not used or archived" do
      ...
      {:ok, _used_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
        raw_text: "Used item",
        status: :used
      })
      {:ok, _archived_item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
  ...
  describe "create_brainstorm_item/1" do
    test "creates a brainstorm item with valid data" do
      ...
      assert {:ok, item} = Composer.create_brainstorm_item(%{
        workspace_id: workspace.id,
        type: :threat,
  ...
  describe "update_brainstorm_item/2" do
    test "updates a brainstorm item with valid data" do
      ...
      assert {:ok, updated_item} = Composer.update_brainstorm_item(item, %{
    ...
    test "returns error changeset with invalid data" do
      ...
      assert {:error, changeset} = Composer.update_brainstorm_item(item, %{
