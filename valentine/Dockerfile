...
# Install Node.js from Debian packages
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/valentine ./
...
CMD ["sh", "-c", "/app/bin/migrate && /app/bin/valentine eval 'Valentine.Seed.run' &&  /app/bin/server"]