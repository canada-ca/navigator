defmodule Valentine.Composer do
  ...
  alias Valentine.Composer.ChatMessage

  @doc """
  Returns the list of chat messages for a workspace and user.

  ## Examples

      iex> list_chat_messages(workspace_id, user_id)
      [%ChatMessage{}, ...]

  """
  def list_chat_messages(workspace_id, user_id) do
    from(cm in ChatMessage,
      where: cm.workspace_id == ^workspace_id and cm.user_id == ^user_id,
  ...
  @doc """
  Deletes all chat messages for a workspace and user.

  ## Examples

      iex> delete_chat_messages(workspace_id, user_id)
      {count, nil}

  """
  def delete_chat_messages(workspace_id, user_id) do
    from(cm in ChatMessage,
      where: cm.workspace_id == ^workspace_id and cm.user_id == ^user_id
    )
    |> Repo.delete_all()
